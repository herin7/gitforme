...
function activate(context) {
    ...
    const loginCommand = vscode.commands.registerCommand('gitforme.login', async () => {
        const success = await (0, api_1.loginWithGitHub)();
        if (success) {
...
class GitformeSidebarProvider {
    ...
    async initializeAuth() {
        try {
            this.currentUser = await (0, api_1.getCurrentUser)();
    ...
    async analyzeRepository(repoUrl) {
        ...
        try {
            ...
            const { username, reponame } = (0, api_1.parseGitHubUrl)(repoUrl);
            if (!username || !reponame) {
                throw new Error('Invalid GitHub URL');
            }
            // Fetch basic repo data
            const repoData = await (0, api_1.fetchRepoData)(username, reponame);
            this._view.webview.postMessage({
                type: 'showRepoData',
                data: {
                    repoData,
